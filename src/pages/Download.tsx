...
const ReleaseCard = ({ release, isLoading, error }) => {
  ...
  if (!release) return <p className="text-center text-gray-400">No release found.</p>;

  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg hover:scale-105 transition-transform duration-300 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6">
        <h3 className="text-xl font-bold mb-2 sm:mb-0">{release.name}</h3>
        <span className={`${release.badgeColor} text-white px-3 py-1 rounded-full text-xs font-bold`}>
          {release.badge}
        </span>
      </div>
...
const Download: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLatestRelease = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('https://api.github.com/repos/rattermc/RinBounce69/releases/latest');
        ...
        setRelease({
          ...
          features: data.body ? data.body.split('\n').filter(line => line.startsWith('- ')).map(line => line.slice(2)) : ['Latest features and improvements'],
        });
      } catch (err) {
